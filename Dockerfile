...
RUN python3.10 facefusion.py --command force-download
...
HEALTHCHECK --interval=30s --timeout=10s --start-period=5s --retries=3 \
    CMD python3.10 -c "import torch; assert torch.cuda.is_available()" || exit 1

# Khởi động handler
CMD ["python3.10", "facefusion_handler.py"]
